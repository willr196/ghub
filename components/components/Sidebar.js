...
export default function Sidebar() {
  ...
  return (
    <aside 
      className={`fixed left-0 top-0 h-screen bg-gradient-to-b from-dark-card to-dark-bg border-r border-white/5 transition-all duration-300 z-50
        ${isOpen ? 'w-64' : 'w-[72px]'}`}
    >
      {/* Header */}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 border-t border-white/5">
        {isAuthenticated ? (
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg border border-success text-success hover:bg-success/10 transition-colors`}
          >
            <span>🔓</span>
          ...
          <Link
            href="/login"
            className={`w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg border border-white/10 text-gray-400 hover:border-primary hover:text-primary transition-colors`}
          >
            <span>🔐</span>
