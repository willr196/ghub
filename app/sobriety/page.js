...
export default function SobrietyPage() {
  ...
  const fetchSobriety = async () => {
    const { data } = await supabase.from('sobriety').select('*').eq('user_id', user.id).eq('is_active', true)
    if (data) setSobrietyData(data)
    setLoading(false)
  }

  const handleSubmit = async (e) => {
    e.preventDefault()
    await supabase.from('sobriety').insert([{ ...formData, user_id: user.id }])
    fetchSobriety()
    setShowForm(false)
  }
      ...
      <main className="flex-1 ml-64 p-8">
        <div className="max-w-5xl mx-auto animate-fadeIn space-y-6">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setShowForm(!showForm)} className="btn-primary">{showForm ? 'Cancel' : '+ Start Tracking'}</button>
          </div>
          ...
          {showForm && (
            <form onSubmit={handleSubmit} className="card space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div><label className="block text-sm text-gray-400 mb-1">Type</label><select value={formData.type} onChange={(e) => setFormData({...formData, type: e.target.value})}><option value="alcohol">Alcohol</option><option value="smoking">Smoking</option></select></div>
                <div><label className="block text-sm text-gray-400 mb-1">Start Date (Day 1)</label><input type="date" value={formData.start_date} onChange={(e) => setFormData({...formData, start_date: e.target.value})} /></div>
              </div>
                  ...
                  <div className="space-y-2">
                    {getBenefits(alcoholDays, 'alcohol').achieved.slice(-3).map((b, i) => (
                      <div key={i} className="flex items-center gap-3 p-3 bg-white/5 rounded-lg border-l-2 border-success text-sm">
                        <span className="text-success">✓</span><span>{b.benefit}</span><span className="ml-auto text-gray-500">Day {b.day}</span>
                      </div>
                    ))}
                  </div>
                  {getBenefits(alcoholDays, 'alcohol').next && (
                    <div className="mt-4">
                      ...
                      <div className="p-3 bg-white/5 rounded-lg border-l-2 border-warning text-sm">
                        ...
                        <span className="block text-gray-500 mt-1">{getBenefits(alcoholDays, 'alcohol').next.day - alcoholDays} days away</span>
                      </div>
                    </div>
                  )}
                </div>
                  ...
                  {getBenefits(smokingDays, 'smoking').next && (
                    <div className="mt-4">
                      ...
                      <div className="p-3 bg-white/5 rounded-lg border-l-2 border-warning text-sm">
                        ...
                        <span className="block text-gray-500 mt-1">{getBenefits(smokingDays, 'smoking').next.day - smokingDays} days away</span>
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="card">
            ...
            <div className="grid md:grid-cols-3 gap-6 text-center">
              <div><span className="text-3xl block mb-2">💵</span><span className="text-2xl font-display font-bold text-success">£{(alcoholDays * 12 + smokingDays * 10).toLocaleString()}</span><span className="block text-gray-400 text-sm">Money Saved</span></div>
              <div><span className="text-3xl block mb-2">❤️</span><span className="text-2xl font-display font-bold text-success">{Math.round((smokingDays * 11) / 60)} hrs</span><span className="block text-gray-400 text-sm">Life Gained</span></div>
              <div><span className="text-3xl block mb-2">🚬</span><span className="text-2xl font-display font-bold text-success">{smokingDays * 20}</span><span className="block text-gray-400 text-sm">Cigarettes Not Smoked</span></div>
            </div>
          </div>
        </div>
      </main>
